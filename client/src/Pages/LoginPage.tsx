...
function LoginPage({ loginStatus, setLoginStatus }: LoginPageProps) {
  ...
  const tryLogin = useCallback(async () => {
    ...
    if (!login.auth) {
      ...
      localStorage.setItem("token", login.token); //stock le token dans une variable locale
      setLoginStatus({
    ...
    <div className="center d-flex flex-column login margin-top-xl">
      ...
      <div className="d-flex flex-column margin-top-xl">
        ...
        <Form.Text id="login" muted>
          If you don't have an account, please contact Zittel Christophe
          at&nbsp;
          <span className="blue">christophe.zittel@al-enterprise.com</span>.
        </Form.Text>
      </div>
    </div>
  );
