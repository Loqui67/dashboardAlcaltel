...
function RegisterPage() {
    ...
    const check = useCallback(() => {
        ...
        if (
            utils.verifUsername(usernameReg) &&
            !utils.hasSpecialCharacters(usernameReg)
        ) {
        ...
        if (
            utils.verifPassword(passwordReg) &&
            passwordReg === passwordRegConfirm
        ) {
    ...
    const renderUsernameTooltip = (props: any) => (
        <Tooltip id="button-tooltip" {...props}>
            <Small
                id="usernameTooltip"
                text="Username size must be 5-20 characters long. No special characters allowed."
            />
        </Tooltip>
    );
    ...
    return (
        <div className="Register d-flex flex-column margin-top-xl">
            ...
            {regState.message && (
                <Alert
                    variant={regState.state ? variant.success : variant.danger}
                    className="alert"
                >
                    {regState.state ? (
