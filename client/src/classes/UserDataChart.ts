...
class UserDataChart {
  ...
  constructor(
    ...
    this.testSuiteFromVersion = Array.from(
      new Map(testSuiteFromVersion.map((item) => [item[key], item])).values()
    );

    this.testSuiteFromVersionWithDate = Array.from(
      new Map(
        testSuiteFromVersionWithDate.map((item) => [item[key], item])
      ).values()
    );
  }
  ...
  getValuesPiePassed() {
    ...
    if (this.dateChoose === "") {
      //retourne le nombre de tests passed
  ...
  getValuesPieFailed() {
    ...
    if (this.dateChoose === "") {
      //retourne le nombre de tests failed
  ...
  getValuesPieSkipped() {
    ...
    if (this.dateChoose === "") {
      //retourne le nombre de tests skipped
      ...
      datasets: [
        {
          label: "test states",
          data: [
            this.getValuesPiePassed()[0],
            this.getValuesPieFailed()[0],
            this.getValuesPieSkipped()[0] /* , this.testStateCount.map((data) => data.not_played) */,
          ] as Array<number>,
          backgroundColor: [
            "rgba(16,251,0, 0.2)",
            "rgba(255,0,0, 0.2)",
  ...
  getBar100Values() {
    ...
    if (this.dateChoose === "") {
      return this.state.map((state) => {
        return this.testSuiteFromVersion
          .filter((data: dataMap) => {
      ...
      return this.state.map((state) => {
        return this.testSuiteFromVersionWithDate
          .filter((data: dataMap) => {
            if (this.clientVersionChoose === 0) {
              return this.allClientID.map((test) => {
                if (test === data.id_client) {
          ...
          .filter((data) => this.dateChoose === data.date)
          .map((element: dataMap) => {
            return this.testState
              .filter(
                (data: dataMap) =>
  ...
  getUserDataBar100Chart() {
    ...
    let labels;
    if (this.dateChoose === "") {
      //on récupère le nom de tte les tests suites en fonctions des filtres
      labels = this.testSuiteFromVersion
        .filter((data: dataMap) => {
          if (this.clientVersionChoose === 0) {
            return this.allClientID.map((test) => {
              if (test === data.id_client) {
      ...
      labels = this.testSuiteFromVersionWithDate
        .filter((data: dataMap) => {
          if (this.clientVersionChoose === 0) {
            return this.allClientID.map((test) => {
              if (test === data.id_client) {
        ...
        .filter(
          (filter: dataMap) =>
            convertDate.getDateAndDeleteHourOnDbFormat(filter.date) ===
            this.dateChoose
        )
        .map(
          (testSuiteFromVersion: dataMap) =>
      ...
      return this.testState
        .filter(
          (data: dataMap) =>
            ...
            data.currentState === state &&
            (convertDate.getDateAndDeleteHourOnDbFormat(data.date) ===
              this.dateChoose ||
              this.dateChoose === "")
        )
        .filter((data: dataMap) => {
