...
class UserDataChart {
  ...
  bgColorFull = ["rgba(23, 222, 23,1)", "rgba(255,0,0,1)", "rgba(248,168,5,1)"];
  bgColorLight = [
    "rgba(23, 222, 23,0.2)",
    "rgba(255,0,0,0.2)",
    "rgba(248,168,5,0.2)",
      ...
      datasets: [
        {
          ...
          backgroundColor: this.bgColorLight.map((element) => element),
  ...
  getBar100Values(params: Array<any>) {
    ...
    return this.state.map((state) => {
      return params
        ...
        .filter(
          (data) => this.dateChoose === data.date || this.dateChoose === ""
        )
        .map((element: dataMap) => {
  ...
  getBar100Labels(params: Array<any>) {
    ...
    return params
      ...
      .map(
        (testSuiteFromVersion: dataMap) => testSuiteFromVersion.testsSuites_name
      );
  }
