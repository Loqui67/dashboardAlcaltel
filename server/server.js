...
function generateHTML(allData, allTS, version, htmlFilePath) {
    ...
    const uniqueTestSuite = [
        ...new Set(allData.map((item) => item.id_testsSuites)),
    ];
            ...
            <div class="flexColumn">
                ...
                ${uniqueTestSuite
                    .map((TS) => {
                        ...
                        <h2>${allTS
                            .filter((oneTS) => TS === oneTS.id_testsSuites)
                            .map((TStitle) => TStitle.testsSuites_name)}</h2>
                        ${
                            allData
                                .filter((data) => data.id_testsSuites === TS)
                                .forEach((element) => {
                                    countTS[element.id_state] =
                                        (countTS[element.id_state] || 0) + 1;
                                }) === undefined
                                ? ""
                                : ""
                        }
                        ...
                        <div class="flexColumn">
                                    ...
                                    <p>Client : ${data.client_name} ${
                                    data.version
                                }</p>
                                    <a href="http://ns3053040.ip-137-74-95.eu:3000/stats/${
                                        data.client_name
                                    }/${version}/${
                                    data.id_testRun
                                }">Show test in dashboard</a>
                                </div>`;
                            })
                            .join("")}`;
                    })
